...
# Simple spin hack script written for Osu back when I was bad at spinners and wanted to test the limites.
...
$screen_width, $screen_height = 1920, 1080 # Screen size settings.
...
set_cursor_pos = Win32API.new("user32", "SetCursorPos", ["I", "I"], "V")
key_press = Win32API.new("user32", "GetAsyncKeyState", ["i"], "i")

def handle_press(keycode)
	if keycode == $spin_key then
		if $spin_active == true then
...
loop do # Main program loop.
	...
	(0x30..0x39).each {
		|keycode|
		if key_press.call(keycode) & 0x01 == 1 then
			if $showKeycodes then
